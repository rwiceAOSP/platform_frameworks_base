...
interface IHealthService {
    ...
    void registerHealthListener(in IHealthListener listener);
    void unregisterHealthListener(in IHealthListener listener);
    IncompatibleChargerData getIncompatibleChargerData();
    HealthData getHealthDataWithAlgo(in int algo);
    boolean setChargingPolicy(in int policy);
    boolean isPulsarEnabled();
    void setPulsarEnabled(in boolean enabled);
}
